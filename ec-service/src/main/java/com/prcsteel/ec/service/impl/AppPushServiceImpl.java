...
import com.prcsteel.ec.service.AppPushService;
...
//			logger.info("Got android result - " + result);
	...
	public PushPayload buildPushObjectIosAliasAlert(IosAlert alert,String alias) {
		return PushPayload.newBuilder().setPlatform(Platform.ios()).setAudience(Audience.alias(alias))
				...
				.setOptions(Options.newBuilder().setApnsProduction(Boolean.parseBoolean(apnsProduction)).build()).build();
	}
...
//                        .addExtra("from", "JPush")
                        .build())
				.setOptions(Options.newBuilder().setApnsProduction(Boolean.parseBoolean(apnsProduction)).build())
                .build();
    }
}
