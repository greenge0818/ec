define(function(require,exports,module){
	...
	var land = {
		...
		init:function(){
			...
			//点击表单清空文本
			land.xqEmptyFormText($("#focastextarea"));
			...
			if($.support.style){
				$(document).on("change","#uploadFile1,#uploadFile2",function(){
					var filetypeTrue = land.fileChange($(this));
					var btnID = $(this).attr("id"),isSizeTrue = land.getFileSize($(this));
			        if(!filetypeTrue){
			            return false;
			        };
			        ...
			        if(btnID == "uploadFile1"){ //判断file
				        ...
				        land.uploadFile("uploadFile1");
			        }else{
				        $("#uploadFile1").attr("name","");
				        $("#uploadFile2").attr("name","uploadFile");
				        land.uploadFile("uploadFile2");
			        }
			        $(".markbtn").hide();
			        $("#upload-c").css("background","#fff");
				})
			}else{
			...
			$(document).on('click', ".input_btn", function () {
		        ...
		        var isTrue = land.checkPhone($("#userPhonetellogin")) ;
	        	if(!isTrue){
	        		return;
	        	}
	        ...
	        $(document).on("change","#userPhonetellogin",function(){
	        	var isTrue = land.checkPhone($(this));
	        	if(!isTrue){
	        		return;
	        	}
			})
	        ...
	        $(document).on('click', ".sendsms", function () {
	           ...
	           var temp = ($(this).attr("data-attr") == "timeLong" ? timeLong : timeLong1)
	            ...
	            land.countDown($(this),($(this).attr("data-attr") == "timeLong" ? timeLong : timeLong1));
	        ...
	        $(document).bind("scroll",function(){
	        	if($(window).scrollTop()>=1810){
	        		$(".ent-qq p").css("color","#fff");
	        	}else{
	        		$(".ent-qq p").css("color","#3d3d3d");
	        	}
	        })
		},
		//检查文件大小
                ...
                success: function(data){
                	if(data.code!="6000"){
                        ...
                        if(imgUrlstr.length <1){
		        			$(".markbtn").show();
		        			$("#upload-c").css("background","#f0f0f0");
	   					}
                        return false;
                    }
                    $(".focaserr-msg-span").hide();
		    ...
		    util.ajax({
				config:{
					...
					success:function(data, status){
						if(data.code == "7000"){
			                gwfinddialog.successDialog(isreload,null,data.status);
    	...
    	var $ = require("jquery"),v = $.verifyElement($("#userPhonetellogin")[0], 3) && $.verifyElement($("#smsCodeNumber")[0], 3),
            ...
            if(rtn == "3012"){
    	 		...
    	 		var config = {
					...
					success:function(data){
						land.checkUserRegCallback(data,2);
					}
				}
...
//				type : 3,
//				logintab : 1,
//				func:function(){land.submitdata(1)}
//			}
