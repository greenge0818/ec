...
/**
 * @Author: Tiny
 * @Description: 找货系统更新需求单状态
 * @Date: 2016年06月02日
 */
public class SmRequirementStatusHandler implements Processor {
    private final Logger log = LoggerFactory.getLogger(SmRequirementStatusHandler.class);
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        MqLog mqLog = new MqLog(MqLogModual.SM_REQUIREMENT_STATUS.getModule(), RemoteDataSource.SMART.getCode(), content, "Y", null);
        if (StringUtils.isNotBlank(content)) {
            try {
                RequirementStatusChangerQuery pick = new Gson().fromJson(content, RequirementStatusChangerQuery.class);
                pick.setSource(RemoteDataSource.SMART.getCode());
                ...
                mqLog.setIsSuccess("N");
                mqLog.setErrorMsg("解析找货单提供的更新需求单失败：" + e.getMessage());
                genericDaoService.insert(mqLog);
            }
        } else {
            mqLog.setContent("无");
            mqLog.setIsSuccess("N");
            mqLog.setErrorMsg("获取找货单提供的更新需求单失败:changeId-" + exchange.getExchangeId());
            genericDaoService.insert(mqLog);
        }
    }
}
